...
**L3-CPP-015** · Parent: L2-CLI-011 · Verification: T
While the C++ implementation is delivered in phases, a flag or subcommand that belongs to an unported module SHALL be refused with a diagnostic naming the missing capability, rather than reported as unknown. Both spellings of such a flag SHALL be refused. An operator transferring a working invocation from another implementation must be able to tell "this build does not have that yet" from "you have mistyped this", because the two have entirely different remedies.
