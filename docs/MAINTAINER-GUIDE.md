...
| `cpp-conformance` | The C++ binary against the **same** byte-exact oracles in `tests/conformance/expected/`, via `run.py --only cpp`. Needs no cargo and no installed `mie_decoder`, because it compares against the committed oracles rather than against the other CLIs. Cases needing the unported merge are skipped **by name** in the log | `ubuntu-24.04`, `windows-2022` | Block merge |
